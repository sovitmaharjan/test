...
<body class="antialiased">
    <form action="{{ route('vimeo') }}" method="post" enctype="multipart/form-data">
        @isset($video_data)
            <iframe src="{{ $video_data['player_embed_url'] }}" width="1200" height="600" frameborder="0"
                allow="autoplay; fullscreen; picture-in-picture" allowfullscreen title="{{ $video_data['name'] }}"></iframe>
        @endisset
        @isset($a)
            @php
                $video0 = $a['body']['data'][0];
                $role = 'admin';
            @endphp
            {{-- {{ dd($video0) }} --}}
            @if ($role == 'admin')
                <iframe src="{{ $video0['player_embed_url'] }}" width="1200" height="600" frameborder="0"
                    allow="autoplay; fullscreen; picture-in-picture" allowfullscreen title="{{ $video0['name'] }}"></iframe>
            @endif
        @endisset
        <br />
        @csrf
